...
public abstract class SimpleGdbImplCommon extends SimpleGdb
{
	...
	final SimpleGdb.QueryLifeCycle qCrossRefsNoDS = new SimpleGdb.QueryLifeCycle (
			"SELECT dest.idRight, dest.codeRight FROM link AS src JOIN link AS dest " +
			"ON src.idLeft = dest.idLeft and src.codeLeft = dest.codeLeft " +
			"WHERE src.idRight = ?"
		);
	...
	final SimpleGdb.QueryLifeCycle qCrossRefsWithCode = new SimpleGdb.QueryLifeCycle (
			"SELECT dest.idRight, dest.codeRight FROM link AS src JOIN link AS dest " +
			"ON src.idLeft = dest.idLeft and src.codeLeft = dest.codeLeft " +
			"WHERE src.idRight = ? AND dest.codeRight = ?"
		);
	...
	final SimpleGdb.QueryLifeCycle qCrossRefsWithCodeNoDS = new SimpleGdb.QueryLifeCycle (
	...
	public Set<Xref> mapID (Xref idc, DataSource... resultDs) throws IDMapperException
	{
		...
		if(idc.getDataSource()!=null)
			pst = resultDs.length != 1 ? qCrossRefs : qCrossRefsWithCode;
		else
			pst = resultDs.length != 1 ? qCrossRefsNoDS : qCrossRefsWithCodeNoDS;
		...
		synchronized (pst) {
			try
			{
				...
				if(idc.getDataSource()!=null) {
					...
					if (resultDs.length == 1)
						pst.setString(2, resultDs[0].getSystemCode());
				}
				
				Set<DataSource> dsFilter = new HashSet<DataSource>(Arrays.asList(resultDs));
	
				//System.out.println("->"+pst.getSQL().toString());
			...
			try
	    	{
	    	 	...
	    	 	while (rs.next()) {
	    	 		...
	    	 		if (DataSource.systemCodeExists(syscode)) {
	    	 			...
	    	 			DataSource ds = DataSource.register(syscode, "Datasource ("+ syscode + ")").asDataSource();
	    	 			i++;
	    	 			result.add (ds);
	    	 		}
	    	 	}
