...
public class IDMapperStack implements IDMapper, AttributeMapper
{
	...
	public void addIDMapper_(IDMapper idMapper){
		addIDMapper(idMapper, null);
	}
	
	public void addIDMapper(IDMapper idMapper, String databaseId){
        if (idMapper!=null) {
            gdbs.add(idMapper);
    ...
    {
    	this.addIDMapper(idMapper, "unknown");
    }
	...
	private Set<Xref> mapIDtransitive(Xref ref, DataSource... resultDs)
		...
		{
			Set<Xref> xrefset = getTransitiveGraph().mapIDtransitiveUntargetted(ref);
			
			//for(Xref xref : xrefset) 
			//	xref.setDatabaseId(this.databaseId);
	...
	private Set<Xref> mapIDnormal(Xref ref, DataSource... resultDs) throws IDMapperException 
	{
		...
		for (IDMapper child : gdbs)
		{
			if (child != null && child.isConnected())
			{  
				Set<Xref> xrefset = child.mapID(ref, resultDs);
				for(Xref xref : xrefset) 
					xref.setDatabaseId(this.databaseId);
				result.addAll (xrefset);
			}
		}
		return result;
	}
